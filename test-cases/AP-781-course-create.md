...
| Finding | Severity | Defect                                                                                                                                                                              | Source                                        | Case    | Issue                                                               |
| ------- | -------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------- | ------- | ------------------------------------------------------------------- |
| D-01    | Medium   | A course cannot be edited: the list's edit button is commented out, its handler pushes `admin.course.create`, which has never existed, and the wizard has no code to load a course. | `views/courses/IndexView.vue:75-85, :159-164` | TC-004  | [#783](https://github.com/blubird-interactiveltd/apical/issues/783) |
| D-02    | Low      | The course name has no length limit in the UI; an over-long name is only caught, if at all, on the final save.                                                                      | `BasicInformationView.vue` validations        | TC-011c | [#784](https://github.com/blubird-interactiveltd/apical/issues/784) |
| D-03    | Medium   | The thumbnail accepts any file: no `accept` on the input and no type check before upload.                                                                                           | `atom/form/FileUploadComponent.vue:25-30`     | TC-012  | [#785](https://github.com/blubird-interactiveltd/apical/issues/785) |
| D-04    | Low      | Online Course Link is not validated; any text is saved as the link.                                                                                                                 | `CourseTypeView.vue:33-37`                    | TC-017  | [#786](https://github.com/blubird-interactiveltd/apical/issues/786) |
| D-05    | Medium   | A list step with nothing selected saves no count, so Review shows a blank instead of 0.                                                                                             | `PtePractice.vue` (and siblings) `submit`     | TC-032  | [#787](https://github.com/blubird-interactiveltd/apical/issues/787) |
| D-06    | Medium   | Duration and prices accept zero, negative and non-numeric values; only "duration required" is checked.                                                                              | `DurationAndPrice.vue` validations            | TC-036  | [#788](https://github.com/blubird-interactiveltd/apical/issues/788) |
| D-07    | Low      | Blank prices render as a lone "$" on Review.                                                                                                                                        | `Review.vue:121, :130`                        | TC-041  | [#789](https://github.com/blubird-interactiveltd/apical/issues/789) |
| D-08    | High     | The description is rendered with `v-html`, so script in it runs on Review (stored XSS if the API keeps it — TC-056b).                                                               | `atom/Content.vue:5`                          | TC-056  | [#790](https://github.com/blubird-interactiveltd/apical/issues/790) |
| D-09    | Medium   | Save has no in-flight guard; a double click creates two courses.                                                                                                                    | `Preview.vue:132, :146`                       | TC-046  | [#791](https://github.com/blubird-interactiveltd/apical/issues/791) |
| D-10    | Low      | A failed create is an unhandled promise rejection (the global toast does show the message).                                                                                         | `Preview.vue:146-165`                         | TC-047  | [#792](https://github.com/blubird-interactiveltd/apical/issues/792) |
| D-11    | Medium   | Materials seeds its `tests` list from `questions`, so its submit drops the practice, mock and quiz ids from the payload's `tests`.                                                  | `Materials.vue` `created()`                   | TC-044b | [#793](https://github.com/blubird-interactiveltd/apical/issues/793) |
